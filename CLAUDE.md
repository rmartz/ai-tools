...
- **Pin every GitHub Action to a full commit SHA** with a `# vX.Y.Z` comment
  (`uses: owner/repo@<40-char-sha> # v7.0.0`), never a mutable tag — a tag can be
  force-moved by a compromised upstream to run code with our token. The version
  comment lets Dependabot's `github-actions` ecosystem keep both the SHA and the
  comment current. Local `./…` action refs are exempt (they move with the commit).
  Enforced by `pnpm run check:actions` (`scripts/check-action-pins.ts` +
